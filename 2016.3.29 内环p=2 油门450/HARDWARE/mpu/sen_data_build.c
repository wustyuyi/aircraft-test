...
void inv_build_quat(long *quat, unsigned long timestamp)
{
	...
	q2 = quat[2] / q30;
	q3 = quat[3] / q30;
	sen.angle.pitch = asin(2 * q0 * q2 - 2 * q1 * q3) * 57.29578f;
	sen.angle.roll = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.29578f;
	sen.angle.yaw = atan2(2 * (q1*q2 + q0*q3), q0*q0 + q1*q1 - q2*q2 - q3*q3) * 57.29578f;
