...
#define MPU9150							//定义我们使用的传感器为MPU6500
...
//		/* Enabled sensors. Uses same masks as fifo_en, NOT pwr_mgmt_2. */
//		st.chip_cfg.sensors=1;		//自己添加
//		
//    if (mpu_read_mem(MPU6500_MEM_REV_ADDR, 1, &rev))
//		{
//				log_e("mpu_read_mem Error........\r\n");
//        return -1;
//		}
//    if (rev == 0x1)
//        st.chip_cfg.accel_half = 0;
...
//        return -1;
//    }
		st.chip_cfg.accel_half = 0; //sun_ds    READ rev==3.....
    ...
    st.chip_cfg.dmp_sample_rate = 0;
		log_i("mpu_set_sensors success........\r\n");
    if (mpu_set_gyro_fsr(2000))  //陀螺仪量程 2000rad/s
        return -1;
    if (mpu_set_accel_fsr(2))  //加速度量程 +-2g
        return -1;
    if (mpu_set_lpf(42))  //内置低通滤波 42Hz
        return -1;
...
//	u8 res=0;
	...
	{	 
		...
		return 1;else log_i("mpu_set_sensor complete ......\r\n");
		if(mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL))//设置FIFO
		return 2;else log_i("mpu_configure_fifo complete ......\r\n");
		if(mpu_set_sample_rate(DEFAULT_MPU_HZ))//设置采样率
		return 3; else log_i("mpu_set_sample_rate complete ......\r\n");
		if(dmp_load_motion_driver_firmware())		//加载dmp固件
		return 4; else log_i("dmp_load_motion_driver_firmware complete ......\r\n");
		if(dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation)))//设置陀螺仪方向
		return 5; else log_i("dmp_set_orientation complete ......\r\n");
		if(dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT|DMP_FEATURE_TAP|	//设置dmp功能
		    DMP_FEATURE_ANDROID_ORIENT|DMP_FEATURE_SEND_RAW_ACCEL|DMP_FEATURE_SEND_CAL_GYRO|
		    DMP_FEATURE_GYRO_CAL))
		return 6; else log_i("dmp_enable_feature complete ......\r\n");
		if(dmp_set_fifo_rate(DEFAULT_MPU_HZ))	//设置DMP输出速率(最大不超过200Hz)
		return 7; else log_i("dmp_set_fifo_rate complete ......\r\n");
		if(run_self_test())		//自检
		return 8; else log_i("run_self_test complete  ......\r\n");
		if(mpu_set_dmp_state(1))	//使能DMP
		return 9; else log_i("mpu_set_dmp_state complete  ......\r\n");
	}
	return 0;
}
