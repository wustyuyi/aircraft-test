...
void DirCtrl(uint16_t xx,uint16_t yy)
{
	...
	if(Tm!=img.time)
	{
		...
		if(sen.high>20 && img.rate>=3)
		{
			...
			{
				DIRX_PID.Angle_Erro=xx-Expect.x+255.45f*(-tan(sen.angle.pitch/57.29578f)-tan(sen.angle.roll/57.29578f));
				DIRY_PID.Angle_Erro=yy-Expect.y+255.45f*(tan(sen.angle.pitch/57.29578f)-tan(sen.angle.roll/57.29578f));
				nt=0;
			}
			if(nt<16)
			{
