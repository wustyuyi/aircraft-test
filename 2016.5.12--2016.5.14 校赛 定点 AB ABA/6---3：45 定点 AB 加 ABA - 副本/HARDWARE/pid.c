...
{	
	PID_Pitch.Angle_Erro = pitch0 + Expect.pitch - _angle_pitch ;
...
{	
	PID_Roll.Angle_Erro = roll0 + Expect.roll - _angle_roll;
...
void Pid_Init()
{
	...
	DIRX_PID.Kp_I=0.025;//0.07;
	DIRX_PID.Ki_I=0.002;
	DIRX_PID.Kd_I=0.03;
	
	DIRY_PID.Kp_I=0.025;//0.07;
	DIRY_PID.Ki_I=0.002;
	DIRY_PID.Kd_I=0.03;
	...
	if(Tm!=img.time)
	{
		...
		if(sen.high>20 && img.rate>=3)
		{
			if(xx==0 || yy==0 || xx>=320 || yy>=240)
			{
				...
				{
					DIRY_PID.Angle_Erro=0;
					DIRX_PID.Angle_Erro=0;
					DIRY_PID.LastAngle_Erro=0;
					DIRX_PID.LastAngle_Erro=0;
					roll0=0;
					pitch0=0;
				}
			}
			...
			{
				DIRX_PID.Angle_Erro=xx-160+255.45f*(-tan(sen.angle.pitch/57.29578f)-tan(sen.angle.roll/57.29578f));
				DIRY_PID.Angle_Erro=yy-120+255.45f*(tan(sen.angle.pitch/57.29578f)-tan(sen.angle.roll/57.29578f));
				nt=0;
			}
			if(nt<16)
			{
				CtrlDirX(&DIRX_PID);
				CtrlDirY(&DIRY_PID);
				pitch0 = DIRX_PID.Output - DIRY_PID.Output;
				roll0 = DIRX_PID.Output + DIRY_PID.Output;
//				
//				pitch0 = 0;
//				roll0 = 0;
			}
...
{
	pPID->Pout_Out = pPID->Angle_Erro * pPID->Kp_I;
	pPID->Dout_Out = (pPID->Angle_Erro - pPID->LastAngle_Erro) * img.rate * pPID->Kd_I/2;
	pPID->Iout_Out += pPID->Angle_Erro * pPID->Ki_I / img.rate;
	pPID->LastAngle_Erro =  pPID->Angle_Erro;
	pPID->Output = _maxmin(pPID->Pout_Out + pPID->Dout_Out + pPID->Iout_Out, 6, -6);
}
...
void LineCtrlYaw(void)
{
	if(img.Line[0].pt1x!=0 || img.Line[0].pt1y!=0)
	{
		if(img.CerR>0 
			&& (_abs(img.Line[0].pt2x - img.Line[0].pt1x) > _abs(img.Line[0].pt1y - img.Line[0].pt2y)
			|| img.Line[0].pt1y>img.CerPty))
		{
			if(img.Line[0].pt2x<img.CerPtx)
				dyaw=45;
			else
				dyaw=-45;
		}
		...
		{
			dyaw=57.3f*atan2(img.Line[0].pt2x-img.Line[0].pt1x, img.Line[0].pt2y-img.Line[0].pt1y);
		}
		yaw0=sen.angle.yaw+_maxmin(dyaw,10,-10);
		if(yaw0>180)
			yaw0-=360;
		if(yaw0<=-180)
			yaw0+=360;
...
void LineCtrl(LINE_S line,short fn)
{
	...
	if(Tm==img.time)
		return;
	Tm=img.time;
	if(sen.high>22 && img.rate>=3)
	{
		if(line.pt1x==0 && line.pt1y==0)
		{
			if(nt<16)
			{
				nt++;
		...
		{
      short xt=(line.pt2x-line.pt1x)*(120-line.pt1y)/(line.pt2y-line.pt1y)+line.pt1x;
      xt=_maxmin(xt,320,0);
			DIRX_PID.Angle_Erro=xt+255.45f*(/*tan*/-(sen.angle.pitch/57.29578f)-/*tan*/(sen.angle.roll/57.29578f))-160;
			if(fn>0)
				DIRY_PID.Angle_Erro=fn<(line.pt2y-120)?fn:(line.pt2y-120);
			else
				DIRY_PID.Angle_Erro=fn>(line.pt1y-120)?fn:(line.pt1y-120);
//			if(DirF==0)
				dyaw=57.3f*atan2(img.Line[0].pt2x-img.Line[0].pt1x, img.Line[0].pt2y-img.Line[0].pt1y);	
//			else
//				dyaw=57.3f*atan2(img.Line[0].pt1x-img.Line[0].pt2x, img.Line[0].pt1y-img.Line[0].pt2y);	
			yaw0=sen.angle.yaw+_maxmin(dyaw,5,-5);
			if(yaw0>180)
				yaw0-=360;
			if(yaw0<=-180)
				yaw0+=360;
