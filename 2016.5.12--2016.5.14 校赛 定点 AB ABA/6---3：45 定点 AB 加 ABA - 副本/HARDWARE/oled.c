...
void OLED_ShowFloat(u8 x,u8 y,double n,u8 size)
{
	...
	if(n == 0)
	{
		...
		while(usize--)
		{
			OLED_ShowChar(x+8*(size-usize+1),y,'0');
		}
		return ;
	}
	do{
	...
	if(oldsize!=(temp+t))
	{
		OLED_Clear(x+8,y,x+8*oldsize,y+1);
		oldsize = temp+t;
	}
	if(n<0)
		OLED_ShowChar(x,y,'-');
		...
		OLED_ShowChar(x,y,'+');
	OLED_ShowNum(x+8,y,n,temp,SIZE,0);
	OLED_ShowChar(x+8*temp+8,y,'.');
	OLED_ShowNum(x+8*temp+8+8,y,num,t-1,SIZE,0);
}
...
{ 	
  ...
  OLED_RST_Set();
	...
	OLED_ShowString(0,0,"Craft is Init...");
